...
# Function: blockBs(x, blockSize, circular = F)
# ---------------------------------
...
# blocks of size "blockSize". The circular argument refers to weather if a block randomly
...
#' @param mafObject The output of the \code{maf} function.
...
#' @param withUncertainty A logical specifying whether to plot the confidence interval
#' around each maf.
#' @param B Number of replications in the confidence interval calculations
#' @param alpha The significance level
#' @param blockSize The block size of the resampled residuals, i.e. the number of 
...
plot.Maf <- function(maf.obj, smoothSpan=30, cexVal=1.5, 
					 nmaf=min(dim(maf.obj$x)[2], 3), 
					 withUncertainty=TRUE, B=100, alpha=0.05, 
					 blockSize=5) {
	...
	n = dim(mafs)[1]
	p = dim(mafs)[2]	
	mafSmooth = filter2(mafs, smoothSpan)[, 1:nmaf]
	
	if (withUncertainty) {

		confLines = 2
		blockSize = 1
		mafStar = array(0, dim=c(B, n, nmaf))
		statStar = matrix(0, p, B)
		x = maf.obj$x		
		xSmooth = filter2(x, smoothSpan)
		residuals = x - xSmooth
		statObs = sort(getSnrEmpir(mafs, smoothSpan), decreasing=T)
		for (i in 1:B){
			residualsStar = blockBs(residuals, blockSize)
			xStar = xSmooth + residualsStar
			mafStarUnsigned = (as.matrix(maf(xStar)$mafs[, 1:nmaf]))
			corMat = cor(mafStarUnsigned, mafSmooth)
			permutation = t(apply(corMat, 1, vectorAbsMaxSign))
			mafStar[i,,] = mafStarUnsigned %*% permutation
			statStar[, i] = sort(getSnrEmpir(maf(xStar)$mafs, smoothSpan), decreasing=T)
		}
		...
		nSignificantMafs = min(which(pval < alpha))
		print(paste("Estimate of # of MAFs in dataset: ", nSignificantMafs))
		output = list(pval=pval, statStar=statStar, statObs=statObs)
	} else {
		output = NULL
	}
	...
	for (i in 1:nmaf) {
		...
		if (withUncertainty) {
			...
			lines(upper, lty=confLines, lwd=1)
			lines(lower, lty=confLines, lwd=1)
